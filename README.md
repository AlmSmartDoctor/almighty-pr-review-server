...
설정 화면에서 레포를 등록하면 GitHub 접근, 로컬 Git 경로, 하네스, 활성 vendor CLI를 자동 검사한다. `리뷰 준비 완료`가 아니면 표시된 원인을 수정한 뒤 `준비 상태 다시 검사`를 실행한다. 레포 이름은 카드에서 수정할 수 있고, 진행 중인 리뷰나 Wiki 생성이 없을 때 등록 레포와 저장 데이터를 삭제할 수 있다.
